...
const placeImageThemes = {
  colon: ["#176b63", "#e0a22b", "Teatro Colon", "Cultura accesible"],
  malba: ["#e0a22b", "#176b63", "MALBA", "Museo accesible"],
  moderno: ["#176b63", "#8fbfb7", "Museo Moderno", "Arte sin barreras"],
  planetario: ["#172c2a", "#e0a22b", "Planetario", "Ciencia accesible"],
  recoleta: ["#8fbfb7", "#176b63", "Recoleta", "Centro cultural"],
  saavedra: ["#176b63", "#e0a22b", "Museo Saavedra", "Historia accesible"],
  usina: ["#e0a22b", "#176b63", "Usina del Arte", "Musica accesible"],
  sanmartin: ["#176b63", "#d7e8e2", "Teatro San Martin", "Teatro accesible"],
  "palacio-libertad": ["#176b63", "#e0a22b", "Palacio Libertad", "Cultura accesible"],
  "parque-estacion": ["#8fbfb7", "#176b63", "Parque Estacion", "Biblioteca y parque"]
};

function placeImageData(place) {
  const [primary, accent, title, subtitle] = placeImageThemes[place.id] || ["#176b63", "#e0a22b", place.name, place.type];
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 506" role="img" aria-label="${title}">
      <defs>
        <linearGradient id="bg" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0" stop-color="#f7faf9"/>
          <stop offset="1" stop-color="#d7e8e2"/>
        </linearGradient>
      </defs>
      <rect width="900" height="506" fill="url(#bg)"/>
      <rect x="72" y="124" width="756" height="262" rx="26" fill="#ffffff" stroke="${primary}" stroke-width="10"/>
      <path d="M104 386h692l-78 58H182z" fill="${primary}"/>
      <circle cx="214" cy="226" r="58" fill="${accent}"/>
      <rect x="322" y="182" width="360" height="34" rx="17" fill="${primary}"/>
      <rect x="322" y="246" width="280" height="28" rx="14" fill="#8fbfb7"/>
      <rect x="322" y="306" width="210" height="28" rx="14" fill="${accent}"/>
      <path d="M214 168c-32 0-58 25-58 57 0 43 58 98 58 98s58-55 58-98c0-32-26-57-58-57z" fill="#ffffff"/>
      <circle cx="214" cy="224" r="21" fill="${primary}"/>
      <text x="450" y="74" text-anchor="middle" font-family="Arial, sans-serif" font-size="42" font-weight="700" fill="${primary}">${title}</text>
      <text x="450" y="460" text-anchor="middle" font-family="Arial, sans-serif" font-size="26" font-weight="700" fill="#ffffff">${subtitle}</text>
    </svg>`;
...
function updateSessionUI() {
  ...
  sessionLabel.textContent = state.userMode === "visitor"
    ...
    : `Sesión: ${name}`;
  sessionLabel.textContent = state.userMode === "visitor"
    ? "Visitante: sin IA, historial ni registro de lugares"
    : `Sesion: ${name}`;
  renderHistory();
}
...
function renderPlaces() {
  ...
  placesList.innerHTML = filteredPlaces.map((place) => `
    <article class="place-card">
      <img class="place-image" src="${placeImageData(place)}" alt="Imagen de ${place.name}" loading="lazy" data-fallback-image="true" />
      <header>
        <div>
          <h3>${place.name}</h3>
...
function openPlace(placeId) {
  ...
  dialogContent.innerHTML = `
    <section class="dialog-place">
      <h2>${place.name}</h2>
      <img class="dialog-image" src="${placeImageData(place)}" alt="Imagen de ${place.name}" data-fallback-image="true" />
      <div class="tag-list">
        ${place.featureLabels.map((feature) => `<span class="tag">${feature}</span>`).join("")}
      </div>
      <div class="meta-grid">
